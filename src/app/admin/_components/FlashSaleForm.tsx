...
export const FlashSaleForm: React.FC<FlashSaleFormProps> = ({
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof FlashSaleFormValues, string>>>({});
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        productIds: (initialData.products as any[]).map(p => typeof p === 'string' ? p : p._id) || [],
      });
    }
  }, [initialData]);
  ...
  const validate = (): boolean => {
    ...
    if (new Date(formData.endTime) <= new Date(formData.startTime)) newErrors.endTime = 'Ngày kết thúc phải sau ngày bắt đầu';
    if (formData.discountPercent < 1 || formData.discountPercent > 100) newErrors.discountPercent = 'Phần trăm giảm giá phải từ 1 đến 100';
    if (formData.maxQuantityPerUser < 1) newErrors.maxQuantityPerUser = 'Giới hạn mua mỗi khách phải lớn hơn 0';
    if (formData.productIds.length === 0) newErrors.productIds = 'Phải chọn ít nhất 1 sản phẩm';
    setErrors(newErrors);
                ...
                <Card>
                    ...
                    <CardContent className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <Input id="name" name="name" value={formData.name} onChange={handleChange} placeholder="V.d: Siêu sale bùng nổ" />
                            {errors.name && <p className="text-red-500 text-sm">{errors.name}</p>}
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="description">Mô tả</Label>
                            <Textarea id="description" name="description" value={formData.description} onChange={handleChange} placeholder="Mô tả ngắn về chương trình flash sale..." />
                        </div>
                    </CardContent>
                </Card>
                <Card>
            ...
            <div className="lg:col-span-1 space-y-8">
                <Card>
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="maxQuantityPerUser">Giới hạn mua mỗi khách</Label>
                            <Input id="maxQuantityPerUser" name="maxQuantityPerUser" type="number" value={formData.maxQuantityPerUser} onChange={handleChange} />
                            {errors.maxQuantityPerUser && <p className="text-red-500 text-sm">{errors.maxQuantityPerUser}</p>}
                        </div>
                    </CardContent>
                </Card>
            </div>
            ...
            {loadingProducts ? (
              <p>Đang tải danh sách sản phẩm...</p>
                ...
                <>
                  ...
                  {errors.productIds && <p className="text-red-500 text-sm mt-2">{errors.productIds}</p>}
                </>
            )}
        </CardContent>
      </Card>
